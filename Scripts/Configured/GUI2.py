...
while True:
    ...
    if event == 'Save':
        ...
        filename = sg.popup_get_file('Save Config As', save_as=True, no_window=True, default_extension='yaml', file_types=(('YAML Files', '*.yaml'),))
        if filename:
            save_config(filename, config_data)
            window['msg'].update('Configuration saved.')

    elif event == 'Load':
        filename = sg.popup_get_file('Load Config From', no_window=True, default_extension='yaml', file_types=(('YAML Files', '*.yaml'),))
        if filename:
